...
// Helper component to fetch and display the user's name
const UserWelcome = () => {
  ...
  useEffect(() => {
    async function getUserData() {
      try {
        ...
        // Uses the 'name' attribute, falls back to email, then empty string
        setDisplayName(attributes.name || attributes.email || '');
      } catch (error) {
        console.log(error);
      }
    }
  ...
  return <span className="text-sm font-bold text-zinc-300">{displayName}</span>;
};
...
function App() {
  ...
  return (
    ...
    <span className="text-xs text-zinc-500 uppercase font-bold">Logged in as</span>
    <UserWelcome />
</div>
                  <button 
                    onClick={signOut}
